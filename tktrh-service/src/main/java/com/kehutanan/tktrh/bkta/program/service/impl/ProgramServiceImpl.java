...
import com.kehutanan.tktrh.bkta.program.repository.ProgramRepository;
...
public class ProgramServiceImpl implements ProgramService {
    ...
    @Override
    public ProgramPageDTO findByFiltersWithCache(String namaProgram, String tahun, List<String> status,
        ...
        // Add filter for tahun if provided
        if (tahun != null && !tahun.isEmpty()) {
            try {
                Integer tahunInt = Integer.parseInt(tahun);
                spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.equal(
                        root.get("tahunPelaksana"), tahunInt));
            } catch (NumberFormatException e) {
                // Handle invalid tahun format
                throw new IllegalArgumentException("Invalid tahun format: " + tahun);
            }
        }
    ...
    public ProgramPageDTO searchWithCache(String keyWord, Pageable pageable, String baseUrl) {
        ...
        if (keyWord != null && !keyWord.isEmpty()) {
            ...
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.or(
                    ...
                    criteriaBuilder.like(criteriaBuilder.lower(root.join("eselon2").get("nama")), searchPattern),
                    criteriaBuilder.like(criteriaBuilder.lower(root.join("status").get("nama")), searchPattern)
            ));
        }
