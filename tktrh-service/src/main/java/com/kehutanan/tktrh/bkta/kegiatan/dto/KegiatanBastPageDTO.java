...
@Data
public class KegiatanBastPageDTO implements Serializable {
     ...
     */
    public KegiatanBastPageDTO() {
        this.embedded = new Embedded(new ArrayList<>());
        this.links = new HashMap<>();
        this.page = new PageMetadata(0, 0, 0, 0);
    }
    
    public KegiatanBastPageDTO(Page<KegiatanBast> page, PagedModel.PageMetadata pageMetadata, List<Link> links) {
        // Set embedded content
        List<KegiatanBastDTO> kegiatanBastList = page.getContent().stream()
                .map(KegiatanBastDTO::new)
                .collect(Collectors.toList());
        this.embedded = new Embedded(kegiatanBastList);
    ...
    public static class Embedded implements Serializable {
        ...
        @JsonProperty("kegiatanBastList")
        private List<KegiatanBastDTO> kegiatanBastList;
        
        public Embedded() {
            this.kegiatanBastList = new ArrayList<>();
        }
        
        public Embedded(List<KegiatanBastDTO> kegiatanBastList) {
            this.kegiatanBastList = kegiatanBastList;
        }
    }
