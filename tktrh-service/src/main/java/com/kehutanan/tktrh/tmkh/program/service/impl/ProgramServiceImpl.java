...
public class ProgramServiceImpl implements ProgramService {
    ...
    public void deleteById(Long id) {
        // Verify program exists before deleting
        findById(id);
    ...
    @Override
    public ProgramPageDTO findByFiltersWithCache(String nama, String anggaran, List<String> status,
        ...
        if (anggaran != null && !anggaran.isEmpty()) {
            try {
                ...
                spec = spec.and((root, query, criteriaBuilder) -> 
                    criteriaBuilder.equal(root.get("totalAnggaran"), anggaranValue));
            } catch (NumberFormatException e) {
                // Handle invalid number format
                // Could log or throw an exception based on requirements
            }
        }
    ...
    public ProgramPageDTO searchWithCache(String keyWord, Pageable pageable, String baseUrl) {
        ...
        if (keyWord != null && !keyWord.isEmpty()) {
            ...
            spec = spec.and((root, query, criteriaBuilder) -> criteriaBuilder.or(
                    ...
                    criteriaBuilder.like(criteriaBuilder.toString(root.get("totalAnggaran")), searchPattern),
                    criteriaBuilder.like(criteriaBuilder.toString(root.get("totalLuas")), searchPattern),
                    criteriaBuilder.like(criteriaBuilder.toString(root.get("tahunPelaksana")), searchPattern)
            ));
        }
