...
@Data
public class GeoServicePageDTO implements Serializable {
     ...
     */
    public GeoServicePageDTO() {
        this.embedded = new Embedded(new ArrayList<>());
        this.links = new HashMap<>();
        this.page = new PageMetadata(0, 0, 0, 0);
    }
    
    public GeoServicePageDTO(Page<GeoService> page, PagedModel.PageMetadata pageMetadata, List<Link> links) {
        // Set embedded content
        List<GeoServiceDTO> geoServiceList = page.getContent().stream()
                .map(GeoServiceDTO::new)
                .collect(Collectors.toList());
        this.embedded = new Embedded(geoServiceList);
    ...
    public static class Embedded implements Serializable {
        ...
        @JsonProperty("geoServiceList")
        private List<GeoServiceDTO> geoServiceList;
        
        public Embedded() {
            this.geoServiceList = new ArrayList<>();
        }
        
        public Embedded(List<GeoServiceDTO> geoServiceList) {
            this.geoServiceList = geoServiceList;
        }
    }
