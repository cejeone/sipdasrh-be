...
import com.kehutanan.rh.bimtek.model.dto.BimtekDTO;

import lombok.Data;

@Data
public class BimtekPageDTO implements Serializable {
     ...
     */
    public BimtekPageDTO() {
        this.embedded = new Embedded(new ArrayList<>());
        this.links = new HashMap<>();
        this.page = new PageMetadata(0, 0, 0, 0);
    }
    
    public BimtekPageDTO(Page<Bimtek> page, PagedModel.PageMetadata pageMetadata, List<Link> links) {
        // Set embedded content
        List<BimtekDTO> bimtekList = page.getContent().stream()
                .map(BimtekDTO::new)
                .collect(Collectors.toList());
        this.embedded = new Embedded(bimtekList);
    ...
    public static class Embedded implements Serializable {
        ...
        @JsonProperty("bimtekList")
        private List<BimtekDTO> bimtekList;
        
        public Embedded() {
            this.bimtekList = new ArrayList<>();
        }
        
        public Embedded(List<BimtekDTO> bimtekList) {
            this.bimtekList = bimtekList;
        }
    }
