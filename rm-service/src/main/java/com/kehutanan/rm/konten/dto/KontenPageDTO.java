...
import com.kehutanan.rm.konten.model.dto.KontenDTO;

import lombok.Data;

@Data
public class KontenPageDTO implements Serializable {
     ...
     */
    public KontenPageDTO() {
        this.embedded = new Embedded(new ArrayList<>());
        this.links = new HashMap<>();
        this.page = new PageMetadata(0, 0, 0, 0);
    }
    
    public KontenPageDTO(Page<Konten> page, PagedModel.PageMetadata pageMetadata, List<Link> links) {
        // Set embedded content
        List<KontenDTO> kontenList = page.getContent().stream()
                .map(KontenDTO::new)
                .collect(Collectors.toList());
        this.embedded = new Embedded(kontenList);
    ...
    public static class Embedded implements Serializable {
        ...
        @JsonProperty("kontenList")
        private List<KontenDTO> kontenList;
        
        public Embedded() {
            this.kontenList = new ArrayList<>();
        }
        
        public Embedded(List<KontenDTO> kontenList) {
