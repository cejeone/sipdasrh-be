package com.kehutanan.ppth.penghijauan.kegiatan.service.impl;
...
import com.kehutanan.ppth.penghijauan.kegiatan.model.KegiatanDokumentasiFoto;
import com.kehutanan.ppth.penghijauan.kegiatan.model.KegiatanDokumentasiVideo;
import com.kehutanan.ppth.penghijauan.kegiatan.model.KegiatanKontrakPdf;
import com.kehutanan.ppth.penghijauan.kegiatan.model.KegiatanLokusShp;
import com.kehutanan.ppth.penghijauan.kegiatan.model.KegiatanRancanganTeknisFoto;
import com.kehutanan.ppth.penghijauan.kegiatan.model.KegiatanRancanganTeknisPdf;
import com.kehutanan.ppth.penghijauan.kegiatan.model.KegiatanRancanganTeknisVideo;
import com.kehutanan.ppth.penghijauan.kegiatan.model.dto.KegiatanDTO;
import com.kehutanan.ppth.penghijauan.kegiatan.repository.KegiatanRepository;
import com.kehutanan.ppth.penghijauan.kegiatan.service.KegiatanService;
import com.kehutanan.ppth.util.FileValidationUtil;

import jakarta.persistence.EntityNotFoundException;

@Service("penghijauanKegiatanService")
public class KegiatanServiceImpl implements KegiatanService {
    ...
    @Override
    public Kegiatan uploadLokusShp(Long id, List<MultipartFile> lokusShpFiles) {
        Kegiatan kegiatan = repository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Kegiatan not found with id: " + id));

        List<KegiatanLokusShp> lokusShps = new ArrayList<>();
        for (MultipartFile file : lokusShpFiles) {
            ...
            KegiatanLokusShp lokusShp = new KegiatanLokusShp();
            lokusShp.setId(idfile);
            lokusShp.setKegiatan(kegiatan);
            lokusShp.setNamaAsli(file.getOriginalFilename());
            lokusShp.setNamaFile(fileName);
            lokusShp.setPathFile(filePath);
            String encodedPath;
            try {
                encodedPath = URLEncoder.encode(filePath, StandardCharsets.UTF_8.toString());
            } catch (UnsupportedEncodingException e) {
            ...
            lokusShp.setViewUrl("/file/view?fileName=" + encodedPath);
            lokusShp.setDownloadUrl("/file/download?fileName=" + encodedPath);
            lokusShp.setContentType(file.getContentType());
            lokusShp.setUkuranMb((double) file.getSize() / (1024 * 1024));
            lokusShp.setUploadedAt(LocalDateTime.now());

            lokusShps.add(lokusShp);

            try {
                minioStorageService.uploadFile(folderName, fileName, file.getInputStream(), file.getContentType());
            } catch (Exception e) {
                throw new RuntimeException("Failed to upload SHP file to storage: " + e.getMessage(), e);
            }
        }

        kegiatan.setKegiatanLokusShps(lokusShps);
        return repository.save(kegiatan);
    }
    ...
    @Override
    @Transactional
    public Kegiatan deleteLokusShp(Long id, List<Long> lokusShpIds) {
        Kegiatan kegiatan = repository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("Kegiatan not found with id: " + id));

        kegiatan.getKegiatanLokusShps().removeIf(lokusShp -> {
            if (lokusShpIds.contains(lokusShp.getId())) {
                try {
                    // Delete file from MinIO storage
                    minioStorageService.deleteFile("", lokusShp.getPathFile());
                    return true; // Remove this SHP from the list
                } catch (Exception e) {
                    throw new RuntimeException("Failed to delete SHP file from storage: " + e.getMessage(), e);
                }
            } else {
