package com.kehutanan.ppth.perijinan.service.impl;
...
import com.kehutanan.ppth.perijinan.model.PerijinanDokumenSertifikatPdf;
import com.kehutanan.ppth.perijinan.model.dto.PerijinanDTO;
import com.kehutanan.ppth.perijinan.repository.PerijinanRepository;
import com.kehutanan.ppth.perijinan.service.PerijinanService;
...
public class PerijinanServiceImpl implements PerijinanService {
    ...
    public void deleteById(Long id) {
        ...
        if (perijinan.getPerijinanDokumenPdfs() != null) {
            for (PerijinanDokumenPdf pdf : perijinan.getPerijinanDokumenPdfs()) {
                try {
        ...
        if (perijinan.getPerijinanDokumenSertifikatPdfs() != null) {
            for (PerijinanDokumenSertifikatPdf pdf : perijinan.getPerijinanDokumenSertifikatPdfs()) {
                try {
                    minioStorageService.deleteFile("", pdf.getPathFile());
                } catch (Exception e) {
                    System.err.println("Failed to delete PDF file: " + pdf.getPathFile() + " - " + e.getMessage());
    ...
    public Perijinan uploadDokumenAwalPdf(Long id, List<MultipartFile> files) {
        ...
        for (MultipartFile file : files) {
            ...
            PerijinanDokumenPdf dokumenAwalPdf = new PerijinanDokumenPdf();
            ...
            perijinan.getPerijinanDokumenPdfs().add(dokumenAwalPdf);

            try {
                minioStorageService.uploadFile(folderName, fileName, file.getInputStream(), file.getContentType());
            } catch (Exception e) {
                perijinan.getPerijinanDokumenPdfs().removeIf(pdf -> pdf.getId().equals(idfile));
    ...
    public Perijinan uploadDokumenBastPdf(Long id, List<MultipartFile> files) {
        ...
        for (MultipartFile file : files) {
            ...
            PerijinanDokumenSertifikatPdf dokumenBastPdf = new PerijinanDokumenSertifikatPdf();
            ...
            perijinan.getPerijinanDokumenSertifikatPdfs().add(dokumenBastPdf);

            try {
                minioStorageService.uploadFile(folderName, fileName, file.getInputStream(), file.getContentType());
            } catch (Exception e) {
                perijinan.getPerijinanDokumenSertifikatPdfs().removeIf(pdf -> pdf.getId().equals(idfile));
                throw new RuntimeException("Failed to upload file to storage: " + e.getMessage(), e);
            }
        }
    ...
    public Perijinan deleteDokumenAwalPdf(Long id, List<String> uuidPdf) {
        ...
        perijinan.getPerijinanDokumenPdfs().removeIf(file -> {
    ...
    public Perijinan deleteDokumenBastPdf(Long id, List<String> uuidPdf) {
        ...
        perijinan.getPerijinanDokumenSertifikatPdfs().removeIf(file -> {
            if (uuidPdf.contains(file.getId().toString())) {
                try {
                    // Delete file from MinIO storage
                    minioStorageService.deleteFile("", file.getPathFile());
